namespace TinyTeam.UI
{
    ...
    public abstract class TTUIPage
    {
        ...
        private static void HideOldNodes()
        {
            ...
            else if (topPage.mode == UIMode.HideOtherAndNeedBack)
            {
                for (int i = m_currentPageNodes.Count - 2; i >= 0; i--)
                {
                    if (m_currentPageNodes[i].mode != UIMode.HideOtherAndNeedBack)
                        m_currentPageNodes[i].Hide();
                }
            }
        }
        ...
        public static void ClosePage()
        {
            ...
            if (closePage.mode == UIMode.HideOtherAndNeedBack)
            {
                m_currentPageNodes.FindAll(p => p.mode == UIMode.HideOtherAndNeedBack).ForEach(m => { m_currentPageNodes.Remove(m); m.Hide(); });
            }
            else
            {
